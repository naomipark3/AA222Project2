...
def make_iter_plot(seeds, y_extractor, ylabel, title, filename, optimizer):
    fig, ax = plt.subplots(figsize=(6, 5))
 
    #Run the chosen optimizer from each IC and plot the per-iteration trace.
    #Lock to the first three default colors so each IC's line and markers stay matched.
    colors = ['C0', 'C1', 'C2']
...
make_iter_plot(
    ...
    optimizer=optimize_with_history,
)
make_iter_plot(
    seeds=[0, 5, 10],
...
make_iter_plot(
    ...
    y_extractor=lambda f, v: f,
    ylabel=r'$f(x)$',
    title=r'simple2: BFGS with Quadratic Penalty, $f$ vs iteration',
    filename='simple2_qp_f.png',
    optimizer=optimize_qp_with_history,
)
make_iter_plot(
    seeds=[0, 5, 10],
    y_extractor=lambda f, v: max(v, 0.0),
    ylabel=r'$\max(0,\, \max_i c_i(x))$',
    title='simple2: BFGS with Quadratic Penalty, violation vs iteration',
    filename='simple2_qp_viol.png',
    optimizer=optimize_qp_with_history,
)
 
print('Saved simple2_al_f.png, simple2_al_viol.png, '
      'simple2_qp_f.png, simple2_qp_viol.png')